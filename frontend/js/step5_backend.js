...
function renderStep5Metrics(bestRows, skipPillsRender = false) {
  ...
  // Banner logic moved to renderStep5Charts to appear below Confusion Matrix
}
...
window.renderStep5Charts = function (bestRows, skipPillsRender = false) {
  ...
  // Render overfit banner below Confusion Matrix
  if (typeof renderOverfitBanner === 'function') {
    const overfitData = {
      overfitSuspected: rowToRender.dataset.overfitSuspected === 'true' || rowToRender.dataset.overfitSuspected === '1',
      perfectScore: rowToRender.dataset.perfectScore === 'true' || rowToRender.dataset.perfectScore === '1',
      cvMean: rowToRender.dataset.cvMean || '0',
      cvStd: rowToRender.dataset.cvStd || '0',
      overfitReason: rowToRender.dataset.overfitReason || ''
    };

    const banner = renderOverfitBanner(overfitData);
    if (banner) {
