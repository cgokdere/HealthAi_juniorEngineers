...
function analyseCSV(parsedData) {
  ...
  if (rows.length < 10) throw new Error(`Your file must have at least 10 rows (patients). This file has ${rows.length}.`);
  ...
  const columns = fields.map(name => {
    ...
    // Numeric detection (strict): count only cells that are fully numeric.
    // This avoids treating values like "2024-01-01", "12mg", or "A12" as numeric.
    const numericPattern = /^-?\d+(?:\.\d+)?$/;
    const numericCells = strValues
      .map(v => v.replaceAll(',', ''))
      .filter(v => numericPattern.test(v));
  ...
  // Require at least one numeric measurement column
  const hasNumeric = columns.some(c => c.type === 'numeric');
  if (!hasNumeric) throw new Error('Your file must contain at least one numeric measurement column.');
  ...
  return {
    ...
    rows: rows.length,
    columns,
    // The initially suggested target (heuristic). This should NOT change when user later picks a different target.
    recommendedTarget: finalTarget ? finalTarget.name : (columns[columns.length - 1]?.name || ''),
...
function renderTargetSelector(ds) {
  ...
  // "Recommended" should reflect the heuristic/default suggestion, not the current user selection.
  const eligibleCols = ds.columns.filter(c => c.type !== 'identifier' && c.type !== 'text');
  const recommended =
    (ds.recommendedTarget && eligibleCols.some(c => c.name === ds.recommendedTarget))
      ? ds.recommendedTarget
      : '';

  sel.innerHTML = eligibleCols
    .map(c => `<option value="${c.name}" ${c.name === ds.targetColumn ? 'selected' : ''}>${c.name}${recommended && c.name === recommended ? ' (recommended)' : ''}</option>`)
    .join('');
...
function loadDefaultDataset(resetSchema) {
  ...
  if (domainLabelEl) domainLabelEl.textContent = domainName;
  if (domainSelectEl && Array.from(domainSelectEl.options).some(o => o.value === domainName)) {
    domainSelectEl.value = domainName;
    if (typeof syncDomainSelectDisplay === 'function') syncDomainSelectDisplay();
  }
    ...
    complete: function (results) {
      ...
      try {
        ...
        if (meta.defaultTarget) {
          ...
          if (tCol) {
            // Domain metadata default = recommended target for this dataset
            ds.recommendedTarget = meta.defaultTarget;
  ...
  reader.onload = function (e) {
    try {
      ...
      if (!parsed.data || parsed.data.length === 0) {
        showUploadError(errMsgEl, errorEl, dz, 'The CSV file appears to be empty or could not be parsed. Your file must have at least 10 rows (patients) and at least one numeric column.');
        return;
      }
...
function updateSchemaBanner() {
  ...
  // Gate the right-side data results until mapping is validated + saved
  const resultsSection = document.getElementById('dataResultsSection');
  // Keep the cards visible (no empty half-page); only gate the contents.
  if (resultsSection) resultsSection.style.display = 'flex';

  if (ok) {
    const ds = loadDataset();
    if (ds) renderDataset(ds);
  } else {
    const wrap = document.getElementById('classBalanceBars');
    if (wrap) wrap.innerHTML = '<div style="color:var(--muted);font-size:13px;padding:8px 0;">Validate &amp; save the Column Mapper to view results.</div>';
    const bannerWrap = document.getElementById('classBalanceBanner');
    if (bannerWrap) bannerWrap.style.display = 'none';
    const tbody = document.getElementById('featuresTableBody');
    if (tbody) tbody.innerHTML = '<tr><td colspan="4" style="color:var(--muted);font-size:13px;padding:12px 0;">Validate &amp; save the Column Mapper to view features.</td></tr>';
  }
}
